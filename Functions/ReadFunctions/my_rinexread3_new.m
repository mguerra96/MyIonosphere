function [obs , obs_header]=my_rinexread3_new(obs_file_dir)
...
ObsSys = headBuffer(contains(headBuffer,"OBS TYPES"),1);

% INSERT OBS TYPES FOR SYSTEM IDENTIFICATION


tmp = char(bodyBuffer);
comment_lines=contains(string(tmp(:,:)),'COMMENT');
tmp=tmp(~comment_lines,:);

time_lines=contains(string(tmp(:,:)),'>');

times=datetime(str2num(TimeOfFirstObs{2}),str2num(tmp(time_lines,8:9)),str2num(tmp(time_lines,11:12)),str2num(tmp(time_lines,14:15)),str2num(tmp(time_lines,17:18)),str2num(tmp(time_lines,20:21)));
numOfSats=str2num(tmp(time_lines,34:35));

SatelliteIDs=tmp(~time_lines,1:3);
...
obs.QZSS=table2timetable(temp_tab(contains(string(temp_tab.prn),"J"),:));
obs.NavIC=table2timetable(temp_tab(contains(string(temp_tab.prn),"I"),:));
