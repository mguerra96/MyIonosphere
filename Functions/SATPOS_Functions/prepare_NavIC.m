function NavICNav=prepare_NavIC(nav)

%prepare NavIC ephemeris for the SatellitePosition function

NavICNav=struct();

for sat=unique(nav.NavIC.SatelliteID)'

    ephe=nav.NavIC(nav.NavIC.SatelliteID==sat,:);
    ...
    fieldname=['I' num2str(sat,'%02d')];
    NavICNav.(fieldname)=ephe_tab;
